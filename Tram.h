...
#ifndef CODEFORCES_TRAM_H
#define CODEFORCES_TRAM_H

#endif //CODEFORCES_TRAM_H
#include <iostream>
using namespace std;
//Linear Kingdom has exactly one tram line. It has n stops, numbered from 1 to n in the order of tram's movement.
// At the i-th stop ai passengers exit the tram, while bi passengers enter it. The tram is empty before it arrives
// at the first stop. Also, when the tram arrives at the last stop, all passengers exit so that it becomes empty.
//
...
// the tram at the i-th stop, and the number of passengers that enter the tram at the i-th stop. The stops are given
// from the first to the last stop in the order of tram's movement.
//
...
//At the second stop, 2 passengers exit the tram (1 passenger remains inside). Then, 5 passengers enter the tram. There
// are 6 passengers inside the tram now.
//At the third stop, 4 passengers exit the tram (2 passengers remain inside). Then, 2 passengers enter the tram. There
// are 4 passengers inside the tram now.
